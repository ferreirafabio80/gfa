...
class BIBFA(object):
    ...
    def update_w(self, X):
        for i in range(0, self.s):      
            ...
            self.sigma_w[i] = np.diag(self.E_alpha[i]) + np.sum(self.E_tau[i]) * self.E_zz
            ...
            tmp1 = 1/np.sqrt(self.E_tau[i])
            cho = np.linalg.cholesky((np.outer(tmp, tmp) * self.E_zz) + 
                (np.identity(self.m) * np.outer(tmp1, tmp1)))
            # Determinant for lower bound    
            detW = -2 * np.sum(np.log(np.diag(cho))) - np.sum(
                np.log(self.E_alpha[i])) - \
                    (self.m * np.sum(np.log(self.E_tau[i])))
            self.Lqw[i] = detW
            invCho = np.linalg.inv(cho)
            self.sigma_w[i] = np.outer(tmp1, tmp1) * np.outer(tmp, tmp) * \
    ...
    def update_z(self, X):
        ...
        for i in range(0, self.s):
            for j in range(0, self.d[i]):
                ...
                ww = self.sigma_w[i] + np.dot(w.T, w)
                self.sigma_z += self.E_tau[i][0,j] * ww 
    ...
    def update_tau(self, X):
        for i in range(0, self.s):         
            ...
            for j in range(0, self.d[i]):
                ...
                x = np.reshape(X[i][:,j], (self.N,1))
                ww = self.sigma_w[i] + np.dot(w.T, w)
                self.b_tau[i][0,j] = self.b0_tau[i] + 0.5 * (np.sum(x ** 2) + 
                    np.sum(ww * self.E_zz) - 2 * np.sum(np.dot(
                        x, w) * self.means_z)) 
            self.E_tau[i] = self.a_tau[i]/self.b_tau[i]
